...
(define cursor-x (lambda () (car cursor)))
(define cursor-y (lambda () (cdr cursor)))

(define set-cursor-x (lambda (x))
  (set-car! cursor x))

(define set-cursor-y (lambda (y))
  (set-cdr! cursor y))
...
(define handle-left (lambda ()
                      (cond ((not menu)
                             (cond ((> x 1)
                                    ...
                                    (set-cursor-x (dec x)))
                                   ((> y 1)
                                    (set! x (string-length (buffer:get (dec (cursor->idx y)))))
                                    (set-cursor-x (string-length (buffer:get (dec (cursor->idx y)))))
                                    (set! y (dec y))
                                    (set-cursor-y (dec y)))))
                            (else
...
(define handle-right (lambda ()
                       (cond ((not menu)
                              (cond ((< x (inc (string-length (buffer:get (cursor->idx y)))))
                                     (set! x (inc x))
                                     (set-cursor-x (inc x)))
                                    ((< y (buffer:size))
                                     (set! x 1)
